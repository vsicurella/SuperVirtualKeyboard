...
def project_version() -> str:
    """MAJOR.MINOR.PATCH from CMakeLists + git-commit-count rev, matching
    cmake/GenerateVersion.cmake so zip names line up with the plugin's version."""
...
def add_to_zip(zf: zipfile.ZipFile, path: Path, arcbase: str) -> None:
    if path.is_dir():
        for f in sorted(path.rglob("*")):
            if f.is_file() or f.is_symlink():
                zf.write(f, f"{arcbase}/{f.relative_to(path.parent).as_posix()}")
    else:
        zf.write(path, f"{arcbase}/{path.name}")


def make_zip(items: list[Path], out: Path, version: str) -> None:
